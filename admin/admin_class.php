...
Class Action {
	...
	function requestId() {
		...
		$save_alumni = $this->db->query("INSERT INTO requesti_id set $data ");
		return 1;
	}
	function uploadCV() {
		extract($_POST);
		if($_FILES['cv']['tmp_name'] != ''){
			$fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['cv']['name'];
			$move = move_uploaded_file($_FILES['cv']['tmp_name'],'assets/uploads/'. $fname);
			
			$career_id = $_POST['career_id'];

			$save_alumni = $this->db->query("INSERT INTO `careers_uploads`(`career_id`, `cv`) VALUES ('$career_id', '$fname')");
	...
	function uni_login() {
		...
		if($qry->num_rows > 0){
			...
			$response["message"] = "Valid Credenials";
			$response["user_type"] = $_SESSION['login_type'];
			return json_encode($response);
		}else{
			$response["status"] = false;
	...
	function delete_user(){
		...
		$delete1 = $this->db->query("DELETE FROM `users` WHERE alumnus_id=".$id);
		if($delete)
			return 1;
	}
	function signup(){
	...
	function update_alumni_acc(){
		...
		$chk = $this->db->query("SELECT * FROM alumnus_bio where id = $id");
		$rowch = $chk->fetch_assoc();
		emailSender (
			$rowch['email'],
			'Email Verified',
			'<h1>Your account has been'.(strval($status) == "0" ? 'Unverified' : 'Verified Successfully').' </h1>'
		);
		if($update)
			return 1;
	}
