                        ...
                        <div class="text-center">
                            <?php if (isset($_SESSION['login_id'])) : ?>
                                <?php if (in_array($_SESSION['login_id'], $cids)) : ?>
                                    <span class="badge badge-primary">Commited to attend</span>
                                <?php else : ?>
                                    <div class="text-center">
                                        <div class="form-check d-inline-block">
                                            <input class="form-check-input" type="radio" name="role" id="guestRadio" value="guest" checked>
                                            <label class="form-check-label" for="guestRadio">
                                            Guest
                                            </label>
                                        </div>
                                        <div class="form-check d-inline-block ml-3">
                                            <input class="form-check-input" type="radio" name="role" id="speakerRadio" value="speaker">
                                            <label class="form-check-label" for="speakerRadio">
                                            Speaker
                                            </label>
                                        </div>
                                    </div>
                                    <button class="btn btn-primary" id="participate" type="button">Interested to attend</button>
                                <?php endif; ?>
                            <?php endif; ?>
                        </div>
                    </div>
...
<!-- Guest Speaker Invitation Section -->
<div class="container mt-4">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title text-center">Become a Guest Speaker</h5>
                    <p class="card-text text-center">Are you interested in becoming a guest speaker at this event? <a href="https://www.google.com/forms/about/" target="_blank">Fill out this form</a> for more information.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    function participate($id) {
        ...
        $.ajax({
            ...
            success: function(resp) {
                if (resp == 1) {
                    alert_toast("Data successfully uplaod", 'success')
                    setTimeout(function() {
                        location.reload()
                    }, 1500)
